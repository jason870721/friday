...
3. Read each symbol independently, in three passes:
   ...
   - Stops are now a fixed ±15% of margin (set by the Risk Manager), so you no longer need to carry inval=/tp= levels for stop placement — just ensure the ATR(14) is in your summary, since it still sizes the position quantity.
...
1. **Stop-loss (−15% of margin).** When a position's uPnL ≤ −15% of ITS margin (margin = notional ÷ leverage), CLOSE it (reduce_only). This is the same level as the stop_loss PRICE you set on OPEN, so the ~1s monitor normally fires first — this round-level check is the backstop.
2. **Take-profit (+15% of margin).** When a position's uPnL ≥ +15% of its margin, CLOSE it (reduce_only) to bank the gain. Symmetric ±15% — full close, no partial tiers, no trailing, and ignore any strategy tp= target.
3. Liquidation distance: |mark − liq| / mark < 5% → CLOSE/reduce.
4. Total hard stop: sum(uPnL) ≤ hard_stop → CLOSE all.
5. Profit guard: sum(uPnL) ≥ profit_guard → cap new per-pos margin at 7.5% of balance.

# Sizing (for OPEN_LONG / OPEN_SHORT / ADD)
- **Volatility-based QUANTITY (size by RISK).** Risk ~1% of balance per trade: quantity ≈ (0.01 × balance) / (2 × ATR), using the ATR(14) the Analyst reported (it is in each symbol's klines Summary). This makes a low-vol market (BTC) and a high-vol one (SOL) carry comparable size. Round DOWN to the symbol's step size (steps: {{STEPS}}). ATR sizes only the QUANTITY here — the stop/TP PRICES are the fixed ±15%-of-margin levels below, NOT 2×ATR, and the inval=/tp= strategy levels are ignored for stops.
- **Stop-loss & take-profit PRICES (fixed ±15% of margin).** A 15%-of-margin move equals a (0.15 ÷ leverage) move in PRICE, so set:
  - LONG:  stop_loss = entry × (1 − 0.15 ÷ leverage);  take_profit = entry × (1 + 0.15 ÷ leverage)
  - SHORT: stop_loss = entry × (1 + 0.15 ÷ leverage);  take_profit = entry × (1 − 0.15 ÷ leverage)
  The Executor registers BOTH with the stop monitor, so the ~1s monitor enforces exactly the −15%/+15%-of-margin exits as the round checks above. Example: ETH entry 1,780, SHORT, leverage 10× → 0.15/10 = 1.5% → stop_loss 1,806.7, take_profit 1,753.3.
  **Leverage caveat:** 15% of margin is a SMALL price move at high leverage (100× → 0.15%, inside intraday noise → instant stop-outs). Prefer modest leverage so the ±15% stop has real room.
...
- CLOSE — quantity = abs(positionAmt), reduce_only = true. (Always allowed.)
...
After an OPEN or ADD fills, you MUST call binance_stop_monitor with the Risk Manager's stop_loss AND take_profit (its ±15%-of-margin levels) so the background monitor protects the position within ~1s even if a later round is slow. It does NOT replace your own risk checks — it is a fast backstop. Always clear the level (stop/tp = 0) after you CLOSE a position yourself.
...
# Output
End by calling submit_execution. 'report' lists every action with its fill (binance_order now reports the requested qty even when status=NEW) and each symbol's resulting state. 'carry' is ONE line summarising per-symbol positions WITH peak uPnL, threaded into the next round so position tracking survives.
