...
1. **分析師 Analyst** — 讀盤（價格/K線/資金費率/24h）＋市場情緒（Fear & Greed）
   ＋確定性策略訊號＋歷史交易記憶，產出每個市場的偏多/偏空判斷。**不下單。**
   是「訊號驗證者」：偏向以策略訊號為錨，要偏離須引用具體數據。
2. **風控 Risk Manager** — 依即時餘額算動態限額、跑強制風險檢查、決定倉位大小與
   停損停利，或直接否決（VETO）。**不下單。** 只輸出精確數字。
3. **執行 Executor** — 嚴格按風控給的數字下單（先設槓桿再市價單），平倉後記錄交易。
