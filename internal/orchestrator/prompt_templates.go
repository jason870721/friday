...
3. Read each symbol independently, in three passes:
   ...
   - **Regime-aware bias rule (MANDATORY):** This gate keys off the REGIME and the "MTF Strategy:" line ONLY — Fear & Greed does NOT gate entries here (it feeds conviction in step 4). When the "Regime:" line shows TRENDING:
     · **4h price BELOW its 4h MA20** (bearish trend): LONG requires "MTF Strategy: LONG"; SHORT requires "MTF Strategy: SHORT". A "Cross-TF:" read on its own is NEVER enough to enter — buying a falling knife or shorting a washout on Cross-TF/F&G alone lost money on EVERY live attempt. If MTF Strategy is NEUTRAL, you WAIT.
     · **4h price ABOVE its 4h MA20** (bullish trend, mirror): LONG requires "MTF Strategy: LONG"; SHORT requires "MTF Strategy: SHORT". Cross-TF alone is not enough; if MTF Strategy is NEUTRAL, you WAIT.
     · When the Regime is RANGING or TRANSITIONAL, all biases are permitted as usual.
     · (This is enforced anyway by the signal-persistence gate below, which requires a non-NEUTRAL MTF Strategy held ≥2 rounds before ANY directional bias — so no entry can ever rest on Cross-TF/F&G alone. The counter-trend-loss problem this used to hard-block is now handled more surgically by per-direction strategy calibration, which zeroes a strategy's confidence on its losing side.)
