...
3. Read each symbol independently, in three passes:
   ...
   - **4h trend-alignment + MTF gate (MANDATORY):** Never trade against the 4h trend, defined by the 4h close vs its 4h MA20. Backtests showed counter-4h-trend entries — especially LONGS bought into a 4h downtrend — were the DOMINANT out-of-sample loss: a "clean" counter-trend move is usually a violent bounce that snaps back and stops you out. Aligning every entry with the 4h trend turned a break-even out-of-sample result clearly positive (and lifted the short-side edge in a bear regime). Fear & Greed does NOT gate entries here (it feeds conviction in step 4).
     · **4h price BELOW its 4h MA20** (downtrend): SHORT only, and SHORT requires "MTF Strategy: SHORT". Do NOT take a LONG bias at all — no falling-knife longs no matter how oversold the dip or how strong the bounce looks; wait for the 4h to reclaim its MA20. A "Cross-TF:" read on its own is NEVER enough.
     ...
     · This holds at EVERY regime (TRENDING / RANGING / TRANSITIONAL) — the 4h trend is the master filter. When MTF Strategy is NEUTRAL in the permitted direction, you WAIT.
     · (The signal-persistence gate below also requires a non-NEUTRAL MTF Strategy held ≥2 rounds before ANY directional bias — so no entry can ever rest on Cross-TF/F&G alone.)
