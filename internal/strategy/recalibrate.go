...
// runs ONCE at startup over ~33 days of 4h history; but regimes shift — a
// strategy that crushed a trending month becomes a loser in chop — so static
// confidences go stale. Every FRIDAY_RECALIBRATE_HOURS this re-fetches 4h×200
// candles per symbol, recomputes the win-rate→confidence map, and installs it via
// SetDefaultCalibration (the same store ConsensusFor reads each round).
//
...
// startup sweep (4h × 200 candles).
const (
	recalibrateInterval = "4h"
	recalibrateLimit    = 200
)
