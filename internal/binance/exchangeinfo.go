...
// TradFiPerpContractType is the contractType exchangeInfo reports for
// stock-linked (traditional-finance) perpetuals, e.g. NVDAUSDT. Trading these
// requires a one-time account agreement (see Client.SignTradFiPerpsAgreement).
const TradFiPerpContractType = "TRADIFI_PERPETUAL"

// SymbolInfo is the subset of /fapi/v1/exchangeInfo friday needs: a symbol's
// tradability status, its contract type, and its LOT_SIZE step (the quantity
// increment orders must round to).
type SymbolInfo struct {
	Symbol       string
	Status       string // "TRADING", "PENDING_TRADING", "SETTLING", "CLOSE", ...
	ContractType string // "PERPETUAL", "TRADIFI_PERPETUAL", "CURRENT_QUARTER", ...
	StepSize     string // from the LOT_SIZE filter, e.g. "0.001"
}

// IsTradFiPerp reports whether this is a stock-linked perpetual that needs the
// signed TradFi-Perps agreement before orders are accepted.
func (s SymbolInfo) IsTradFiPerp() bool { return s.ContractType == TradFiPerpContractType }
...
func (c *Client) ExchangeInfo(ctx context.Context) ([]SymbolInfo, error) {
	...
	for _, s := range resp.Symbols {
		info := SymbolInfo{Symbol: s.Symbol, Status: s.Status, ContractType: s.ContractType}
